...
from .models import FTPLog, OperateLog, PasswordChangeLog, UserLoginLog, UpFileLog, IISLog, CrontabLog
...
class UpfileLogListView(AdminUserRequiredMixin, DatetimeSearchMixin, ListView):
    model = UpFileLog
    template_name = 'audits/upfile_log_list.html'
    paginate_by = settings.DISPLAY_PER_PAGE
    user = asset = system_user = filename = ''
    date_from = date_to = None
    ...
    def get_context_data(self, **kwargs):
        context = {
            'user_list': UpFileLog.objects.values_list('user', flat=True).distinct(),
            'asset_list': UpFileLog.objects.values_list('asset', flat=True).distinct(),
            'system_user_list': UpFileLog.objects.values_list('system_user', flat=True).distinct(),
            'date_from': self.date_from,
            'date_to': self.date_to,
            'user': self.user,
            'asset': self.asset,
...
class IisLogListView(AdminUserRequiredMixin, DatetimeSearchMixin, ListView):
    model = IISLog
    template_name = 'audits/iis_log_list.html'
    paginate_by = settings.DISPLAY_PER_PAGE
    user = asset = system_user = filename = ''
    date_from = date_to = None
    ...
    def get_context_data(self, **kwargs):
        context = {
            'user_list': IISLog.objects.values_list('user', flat=True).distinct(),
            'asset_list': IISLog.objects.values_list('asset', flat=True).distinct(),
            'system_user_list': IISLog.objects.values_list('system_user', flat=True).distinct(),
            'date_from': self.date_from,
            'date_to': self.date_to,
            'user': self.user,
            'asset': self.asset,
...
class CrontabLogListView(AdminUserRequiredMixin, DatetimeSearchMixin, ListView):
    model = CrontabLog
    template_name = 'audits/crontab_log_list.html'
    paginate_by = settings.DISPLAY_PER_PAGE
    user = asset = filename = ''
    date_from = date_to = None
    ...
    def get_context_data(self, **kwargs):
        context = {
            'user_list': CrontabLog.objects.values_list('user', flat=True).distinct(),
            'asset_list': IISLog.objects.values_list('asset', flat=True).distinct(),
            'date_from': self.date_from,
            'date_to': self.date_to,
            'user': self.user,
